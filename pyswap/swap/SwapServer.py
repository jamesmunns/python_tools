...
class SwapServer(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            self.modem = SerialModem(portname=self._xmlserial.port,
                                     speed=self._xmlserial.speed,
                                     invert=self._xmlserial.invert,
    ...
    def queryMoteRegisterNoRetry(self, mote, regId, maxWaitMs=2000):
        """
        Query mote register, wait for response and return value
        Non re-entrant method!!
        ...
        register.sendSwapQuery()
        # Wait for aknowledgement from mote
        regVal = self._waitForReg(register, maxWaitMs)
